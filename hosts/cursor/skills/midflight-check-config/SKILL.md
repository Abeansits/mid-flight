...
# MidFlight Config Check (Cursor host)
...
## Note on Cursor hosts

There is no `provider=cursor` yet, so the default `provider=codex` (or whichever CLI you configured) is the expected external consult target. Confirm that provider's CLI is on `PATH` and authenticated.
