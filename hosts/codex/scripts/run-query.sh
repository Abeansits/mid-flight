#!/usr/bin/env bash
# Codex host adapter: resolve engine, apply circular-provider guard, run query.
#
...
#   --allow-codex-provider   allow provider=codex on the Codex host
#   --provider NAME         force provider (implies allow when NAME=codex)
...
while [ $# -gt 0 ]; do
  case "$1" in
    --allow-codex-provider) ALLOW_CODEX=true; shift ;;
    --provider)
      FORCE_PROVIDER="${2:-}"
...
if [ $# -lt 1 ]; then
  printf 'Usage: run-query.sh [--allow-codex-provider] [--provider NAME] <query-file> [mode] [video-prompt]\n' >&2
  exit 2
fi
...
if [ -n "$FORCE_PROVIDER" ]; then
  if [ "$FORCE_PROVIDER" = "codex" ] && [ "${MIDFLIGHT_ALLOW_CODEX_PROVIDER:-}" != "1" ]; then
    printf 'run-query: --provider codex on Codex host requires --allow-codex-provider or MIDFLIGHT_ALLOW_CODEX_PROVIDER=1\n' >&2
    exit 1
  fi
  provider="$FORCE_PROVIDER"
else
  provider="$(bash "$SCRIPT_DIR/prefer-non-codex-provider.sh")"
fi
