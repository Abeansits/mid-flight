...
RESOLVE="$ROOT_DIR/hosts/cursor/scripts/resolve-engine.sh"

# From skill-like start dir inside the real repo tree → finds engine root
out="$(bash "$RESOLVE" --start-dir "$ROOT_DIR/hosts/cursor/skills/midflight")"
kind="${out%% *}"
path="${out#* }"
...
out="$(bash "$RESOLVE" --check --start-dir "$ROOT_DIR/hosts/cursor/skills/midflight")"
kind="${out%% *}"
path="${out#* }"
...
echo "PASS: host_cursor_resolve_engine"
