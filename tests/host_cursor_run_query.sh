...
RUN="$ROOT_DIR/hosts/cursor/scripts/run-query.sh"
SKILL_RUN="$ROOT_DIR/hosts/cursor/skills/midflight/scripts/run.sh"

# Default provider=codex is fine from Cursor — no circular rewrite
...
assert_eq "host-codex-ok" "$result" "Cursor host should use configured codex provider"
...
echo "PASS: host_cursor_run_query"
