...
RESOLVE="$ROOT_DIR/hosts/grok/scripts/resolve-engine.sh"

# From skill-like start dir inside the real repo tree → finds engine root
out="$(bash "$RESOLVE" --start-dir "$ROOT_DIR/hosts/grok/skills/midflight")"
kind="${out%% *}"
path="${out#* }"
...
out="$(bash "$RESOLVE" --check --start-dir "$ROOT_DIR/hosts/grok/skills/midflight")"
kind="${out%% *}"
path="${out#* }"
...
echo "PASS: host_grok_resolve_engine"
